...
- `openCodeSyntheticProviderName`: The name used to register the provider **in OpenCode** when connecting directly to Synthetic. No default — must be set explicitly to enable the direct provider. This is the name you select when running `opencode provider login`.
...
| `openCodeSyntheticProviderName` | string | —                                     | Name for the direct Synthetic provider in OpenCode (required to enable direct mode) |
...
3. If `openCodeSyntheticProviderName` is set, it registers a direct Synthetic provider in OpenCode
...
3. Type the provider name matching your config (e.g., `synthetic` for direct mode, or `synthetic-plexus` for Plexus mode — see `openCodeSyntheticProviderName` and `openCodePlexusProviderName`)
4. Enter the API key:
