...
describe("buildProviderConfig", () => {
  it("returns correct npm package", () => {
    const result = buildProviderConfig([], "http://localhost:8080/v1", "my-provider");
    expect(result.npm).toBe("@ai-sdk/openai-compatible");
  });

  it("uses the provided provider name", () => {
    const result = buildProviderConfig([], "http://localhost:8080/v1", "custom-name");
    expect(result.name).toBe("custom-name");
  });

  it("sets baseURL in options", () => {
    const result = buildProviderConfig([], "http://localhost:8080/v1", "test");
  ...
  it("maps single model to config", () => {
    ...
    const result = buildProviderConfig(models, "http://localhost:8080/v1", "test");
  ...
  it("applies modelOptions to matching alias names", () => {
    ...
    const result = buildProviderConfig(models, "http://localhost:8080/v1", "test", modelOptions);
  ...
  it("handles multiple models with different providers", () => {
    ...
    const result = buildProviderConfig(models, "http://localhost:8080/v1", "multi");
